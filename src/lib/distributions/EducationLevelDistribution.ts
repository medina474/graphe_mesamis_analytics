...
export class EducationLevelDistribution implements Distribution<Education> {
  sample(person: Partial<Person>): Education {
    ...
    let probs = base.map((b, i) =>
      Math.min(0.9, Math.max(0.01, b + adjust[i])),
    );

    return Random.weightedChoice(
      [Education.None, Education.CAP, Education.Bac, Education.Superior],
      probs,
    );
  }
}
